...
def wear_percent(km_since_service: float, interval: float) -> float:
    """Return how worn a car is as a percentage of its service interval."""
    return (km_since_service / interval) * 100


def needs_service(car: dict) -> bool:
    """Return True if the car has reached or exceeded the wear threshold."""
...
def check_fleet(fleet: list) -> list:
    """Flag and print every car in the fleet that is due for service."""
    flagged = []
    for car in fleet:
